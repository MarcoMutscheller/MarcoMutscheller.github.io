...
class Loop {
  ...
  start(time, sync = true) {
    ...
    activeLoops.add(this);
    this.buttoninter.classList.add('active');
  }
  ...
  stop(time) {
    ...
    activeLoops.delete(this);
    this.buttoninter.classList.remove('active');
    this.buttoninter.style.opacity = 0.25;
  }
  ...
  displayIntensity() {
    ...
    if (analyser.getFloatTimeDomainData) {
      ...
      const opacity = Math.min(1, 0.25 + 10 * Math.sqrt(sum / fftSize));
      this.buttoninter.style.opacity = opacity;
    }
  }
...
function loadLoops() {
  ...
  for (let i = 0; i < sounds.length; i++) {
    ...
    request.addEventListener('load', () => {
      decodeContext.decodeAudioData(request.response, (buffer) => {
        const buttoninter = document.querySelector(`div.buttoninter[data-index="${i}"]`);
        loops[i] = new Loop(buffer, buttoninter, levels[i])
      });
    });
